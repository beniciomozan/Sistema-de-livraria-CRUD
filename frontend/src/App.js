...
const App = () => {
  ...
  const [livro, setLivro] = useState({ isbn: "", titulo: "", autor: "" });
  ...
  const fetchLivros = async () => {
    try {
      const res = await axios.get("http://localhost:5432/api/livros");
      setLivros(res.data);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      if (editando && livro.isbn) {
        await axios.put(`http://localhost:5432/api/livros/${livro.isbn}`, livro);
      } else {
        await axios.post("http://localhost:5432/api/livros", livro);
      }

      setLivro({ isbn: "", titulo: "", autor: "" });
      setEditando(false);
  ...
  const handleDelete = async (isbn) => {
    try {
      await axios.delete(`http://localhost:5432/api/livros/${isbn}`);
      fetchLivros(); // Atualiza a lista de livros após a exclusão
    } catch (error) {
      console.error("Erro ao excluir o livro: (trsite)", error);
          ...
          {livros.map((livro) => (
            <tr key={livro.isbn}>
              ...
              <td>
                ...
                <button onClick={() => handleDelete(livro.isbn)}>Excluir</button>
              </td>
            </tr>
          ))}
        </tbody>
